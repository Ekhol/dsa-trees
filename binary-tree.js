...
class BinaryTree {
  ...
  maxSum() {
    ...
    function sumVal(node) {
      if (node === null) {
        return 0;
      }
      const leftSum = sumVal(node.left);
      const rightSum = sumVal(node.right);
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      let cur = queue.shift();
      let val = cur.val;
      let higherThanLB = val > lowerBound;
      let closerVal = val < nearVal || nearVal === null;

      if (higherThanLB && closerVal) {
        nearVal = val;
      }
